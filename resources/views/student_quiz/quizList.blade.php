@foreach($quizList as $qdata)
    @extends('layouts.student_material')
@endforeach

@section('content')
    <div ><p>Today is {{ Carbon\Carbon::now('Asia/Colombo') }} </p>
    </div>
    <!--container-->
                ...
                <div class="card-body" style="background-color:#fd7e1410">
                    <div class="card mb-3">
                            <div class="card-header position-relative">
                                ...
                                <a href="{{route('student.showquiz',$item['id']) }}" style="text-decoration: none;" >
        ...
        @if(isset($missedquizarr) && !empty($missedquizarr))
            @foreach($missedquizarr as $item)
                @php 
                    ...
                    $now = Carbon\Carbon::now('Asia/Colombo');
                @endphp
                <div class="card-body" style="background-color:#dc354525">
                    <div class="card mb-3">
                            <span class=" card-header h3 my-auto">{{$item['title']}}</span>
                            <div class=" card-body row row-cols-2 ">
                                <span class="h5 col ">From  :  {{$item['updated_at']}}</span>
