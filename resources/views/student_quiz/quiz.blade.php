...
@section('content')
    <div class="container-fluid">
            <form  method="POST" enctype="multipart/form-data" id="" action="{{ route('student.checkquiz',[$quiz_id]) }}" class="form-check"> @csrf
                <div class="card w-75 ps-0 ms-0">
                ...
                @endforeach
                        <div class="card-footer text-center">
                            <button type="Submit" class="btn btn-primary btn-l my-auto">Submit</button>
                        </div>
                </div>
            </form>   
        </div>
...
@section('scripts')
    <script src="{{ asset('assets/front/js/student_js/countdown_timer.js') }}"></script> 
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.js" integrity="sha512-n/4gHW3atM3QqRcbCn6ewmpxcLAHGaDjpEBu4xZd47N0W2oQ+6q7oc3PXstrJYXcbNU1OHdQ1T7pAP+gi5Yu8g==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script type="text/javascript">
        console.log("hello")
        console.log(timetoseconds('{{ $quiz->duration}}'))
        var time = '00:00:05'
        ...
        {$.ajax({
                    url: "{{route('student.showquiz',[$quiz->id])}}",
                    cache:false,
                    type: "GET",
                    data:timeobj,
                }).done(function(data) {
